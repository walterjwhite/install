...
_import() {
	if [ $(printf "$3" | grep -c '^git:') -gt 0 ]; then
		...
		*.feature/*)
			...
			local feature_platform_import=$(printf '%s' "$target_file" | sed -e 's/\.feature/.feature\/$_PLATFORM\/lib/')
	...
	*.feature/*)
		...
		local feature_platform_import=$(printf '%s' "$3" | sed -e 's/\.feature/.feature\/$_PLATFORM\/lib/')
